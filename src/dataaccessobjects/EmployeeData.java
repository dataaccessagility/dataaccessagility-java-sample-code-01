...
{
	/**
	* Inserts a single record into database. The values inserted are <code>{@link Employee#id id}</code>, <code>{@link Employee#name name}</code>, <code>{@link Employee#baseSalary baseSalary}</code>, <code>{@link Employee#hireDate hireDate}</code>, <code>{@link Employee#divisionId divisionId}</code>
	...
	*/
	public static int addEmployee(Employee obj) throws Exception
	{
		...
		try (Connection conn = ConnectionProvider.getConnection(); PreparedStatement ps = conn.prepareStatement("INSERT INTO HR_EMPLOYEE (DIVISION_ID, EMPLOYEE_BASE_SALARY, EMPLOYEE_HIRING_DATE, EMPLOYEE_ID, EMPLOYEE_NAME)  VALUES (?, ?, ?, ?, ?)  ;    "))
		{
			ps.setInt(1, obj.getDivisionId());
			ps.setBigDecimal(2, obj.getBaseSalary());
			ps.setDate(3, new Date(obj.getHireDate().getTime()));
	...
	*/
	public static int addEmployee(IDataAccessTransaction txn, Employee obj) throws Exception
	{
		...
		try (PreparedStatement ps = ((DataAccessTransaction)txn).getConnection().prepareStatement("INSERT INTO HR_EMPLOYEE (DIVISION_ID, EMPLOYEE_BASE_SALARY, EMPLOYEE_HIRING_DATE, EMPLOYEE_ID, EMPLOYEE_NAME)  VALUES (?, ?, ?, ?, ?)  ;    "))
		{
	...
	/**
	* Retrieves a single record from database. The values returned are <code>{@link Employee#id id}</code>, <code>{@link Employee#name name}</code>, <code>{@link Employee#baseSalary baseSalary}</code>, <code>{@link Employee#hireDate hireDate}</code>, <code>{@link Employee#divisionId divisionId}</code>
	...
	*/
	public static Employee getEmployeeById(int byId) throws Exception
	{
		...
		try (Connection conn = ConnectionProvider.getConnection(); PreparedStatement ps = conn.prepareStatement("SELECT EMPLOYEE_ID, EMPLOYEE_NAME, EMPLOYEE_BASE_SALARY, EMPLOYEE_HIRING_DATE, DIVISION_ID  FROM HR_EMPLOYEE  WHERE (HR_EMPLOYEE.EMPLOYEE_ID = ?)  ;  "))
		{
			ps.setInt(1, byId);
			...
			if (rs.next())
			{
				...
				retVal.setHireDate(rs.getDate("EMPLOYEE_HIRING_DATE"));
				retVal.setDivisionId(rs.getInt("DIVISION_ID"));
			}
			else
			{
				retVal = null;
	...
	/**
	* Retrieves multiple records from database. The values returned for each record are <code>{@link Employee#id id}</code>, <code>{@link Employee#name name}</code>, <code>{@link Employee#baseSalary baseSalary}</code>, <code>{@link Employee#hireDate hireDate}</code>, <code>{@link Employee#divisionId divisionId}</code>
	...
	*/
	public static java.util.ArrayList<Employee> getAllEmployees(int rowsOffset, int rowsCount) throws Exception
	{
		...
		try (Connection conn = ConnectionProvider.getConnection(); PreparedStatement ps = conn.prepareStatement("SELECT EMPLOYEE_ID, EMPLOYEE_NAME, EMPLOYEE_BASE_SALARY, EMPLOYEE_HIRING_DATE, DIVISION_ID  FROM HR_EMPLOYEE  ORDER BY EMPLOYEE_HIRING_DATE DESC  LIMIT ?, ?  ;  "))
		{
			...
			while (rs.next())
			{
				...
				obj.setHireDate(rs.getDate("EMPLOYEE_HIRING_DATE"));
	...
	* @param baseSalary Employee's salary not including bonuses, housing allowance, or any other allowances
	* @param byId (filter parameter) No documentation is available
	* @return Number of rows affected
	*/
	...
	*/
	public static int addEmployeesBulk(java.util.List<Employee> objs) throws Exception
	{
		...
		{
			ps.setString(1, Employee.ToXml(objs, Employee.ID_BIT_FLAG | Employee.NAME_BIT_FLAG | Employee.BASESALARY_BIT_FLAG | Employee.HIREDATE_BIT_FLAG | Employee.DIVISIONID_BIT_FLAG));
